...
(() => {
  ...
  let themePreference = read('homepage-theme');
  if (!['auto', 'light', 'dark'].includes(themePreference)) themePreference = 'auto';
  const resolveTheme = () => themePreference === 'auto' ? (systemTheme.matches ? 'dark' : 'light') : themePreference;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const labels = { zh: { auto: '自动', light: '白天', dark: '夜晚' }, en: { auto: 'Auto', light: 'Light', dark: 'Dark' } };
    // 静态 SVG 图形，不依赖外部图标库。Auto 根据系统显示太阳或月亮。
    const icons = {
    ...
    function renderButtons() {
      ...
      const nextMode = modes[(modes.indexOf(themePreference) + 1) % modes.length];
      themeLabel.textContent = labels[language][themePreference];
      iconShape.innerHTML = icons[theme];
      themeButton.dataset.mode = themePreference;
      const currentName = themePreference === 'auto' && language === 'zh' ? '自动' : labels[language][themePreference];
      const nextName = nextMode === 'auto' && language === 'zh' ? '自动' : labels[language][nextMode];
    ...
    themeButton.addEventListener('click', () => {
      ...
      theme = resolveTheme();
      root.dataset.theme = theme;
      save('homepage-theme', themePreference);
      renderButtons();
    });
